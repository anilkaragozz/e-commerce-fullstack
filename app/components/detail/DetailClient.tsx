  ...
  return (
    <div className="my-10">
      <PageContainer>
        <div className="block md:flex gap-10 justify-center">
            ...
            <div className="w-1/2 space-y-3">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {
                        product.inStock ? 
                        <div className="text-green-500">ürün stokta</div>
                        : <div className="text-red-500">stokta değil</div>
                    }
                </div>
                <div className="text-lg md:text-2xl text-orange-600 font-bold" > {product.price} ₺ </div>
                {
                    displayButton ? <>
                    <Button text={"product already in basket"} outline onClick={() => {}} />
                    </> : <>
                    <Counter increaseFunc={increaseFunc} decreaseFunc={decreaseFunc} cardProduct={cardProduct} />
                    <Button text={"Add to chart"} small onClick={() => addToBasket(cardProduct)} />
                    </>
                }
            </div>
        </div>
        ...
        <div> 
            {
                product?.rewievs?.map((prd: any) => <Comment key={prd.id} prd={prd} />)
            }
        </div>
      </PageContainer>
    </div>
